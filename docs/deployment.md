...
The project selects Node 22 in `.node-version`, `.nvmrc`, and CI. Run `nvm use`
before `npm ci` in a local checkout. Vite 8 resolves TypeScript paths through
`resolve.tsconfigPaths`, without a separate plugin. The dependency lockfile is
validated by `npm ci` with the npm 10 bundled with Node 22. If npm 10.9.8
fails internally while running `npm audit fix` on this tree, run that command
with a Node-22-compatible npm 11 and validate the result again with `npm ci`.
