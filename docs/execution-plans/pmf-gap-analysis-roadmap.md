...
**Issue #8 item 5 (2026-09-15)**: solo workflow di richiesta + annullamento autenticato (modello request-only della decisione 0006) — NON è remediation dell'item: nessun dato viene cancellato, il flag `deletionRequestedAtMs` resta senza consumer e l'erasure pipeline resta fuori scope (decisione 0015). Fasi svolte: annullamento autenticato e no-op senza richiesta; pulsante e toast EN/IT; entrambe le guide corrette; decisione 0015 registrata; regressioni e TypeScript verificati. Nessun cron o purge: una scadenza senza erasure reale sarebbe una promessa non implementata.
